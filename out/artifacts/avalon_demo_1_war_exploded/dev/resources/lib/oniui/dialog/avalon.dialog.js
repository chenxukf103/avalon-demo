 ...
 */
define(["avalon",
    "text!./avalon.dialog.html"/*"../avalon.getModel",
    ...
    var widget = avalon.ui.dialog = function (element, data, vmodels) {
        ...
        var vmodel = avalon.define(data.dialogId, function (vm) {
            ...
            vm._open = function (updateZIndex) {
                ...
                //suyc add 20160824
                vmodel.returnOriginButton = false;
            ...
            vm._fullScreen = function () {
                ...
                resetCenter(vmodel,element);
                vmodel.fullScreenButton = false;
                vmodel.returnOriginButton = true;
            }
            //suyc add
            vm._returnOrigin = function () {
               if (vmodel.returnDatas.length > 0) {
                   vmodel.fullScreen = false;
                   resetCenter(vmodel,element);
                   vmodel.fullScreenButton = true;
                   vmodel.returnOriginButton = false;
                }
    ...
    function resetCenter(vmodel, target, scroll) {
        ...
        if(vmodel.returnOriginButton&&vmodel.returnDatas.length>0) {
            targetOffsetWidth = vmodel.returnDatas[0].targetOffsetWidth
            targetOffsetHeight = vmodel.returnDatas[0].targetOffsetHeight
        }else {
        ...
        //sucy --addd
        var element_content = target.firstChild.children[1];
        var $element_content = avalon(element_content);
        if(vmodel.fullScreen) {
            target.style.width = "100%";
            var element_header = target.firstChild.children[0];
            var element_footer = target.firstChild.children[2];
            ...
            target.style.width = vmodel.width+"px";
            $element_content.css({height: "auto"});
        }
    }
